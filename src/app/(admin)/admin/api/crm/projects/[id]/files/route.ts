...
export async function GET(_req: NextRequest, { params }: Params) {
  ...
  try {
    ...
    return crmCatch(error, "Dateien konnten nicht geladen werden.");
  }
}
...
export async function POST(req: NextRequest, { params }: Params) {
  ...
  try {
    ...
    const visibleRaw = formData.get("visibleToCustomer");
    const visibleToCustomer =
      category === "internal"
        ? false
        : visibleRaw == null
          ? true
          : String(visibleRaw) === "true";

    if (!file) return crmError("Keine Datei.", 400);
    ...
    return crmCatch(error, "Upload fehlgeschlagen.");
  }
}
